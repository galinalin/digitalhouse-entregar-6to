...
def buscarUsuarioPorDni(nroBuscado):
    ...
    for usuario in usuarios:
        if usuario["Dni"] == nroBuscado:
            ...
            return usuario
    print("Usuario no encontrado. DNI incorrecto.")
    return None

# FUNCIÓN PARA CONSULTAR EL SALDO
def consultarSaldo(usuario):
    print("\nConsultando Saldo...")
    print("💰 Su saldo actual es: $" + str(usuario["Saldo"]))
...
def consultarFacturas(usuario):
    ...
    if len(facturas) == 0:
        print("✅ No tiene facturas adeudadas.")
    else:
        print("📄 SUS FACTURAS ADEUDADAS SON:")
        for factura in facturas:
            print("   - " + factura["Servicio"] + " | Vence: " + factura["Vencimiento"] + " | Valor: $" + str(factura["Valor"]))
...
def pagarFacturas(usuario):
    ...
    facturas = usuario["FacturasAdeudadas"]
    if len(facturas) == 0:
        print("✅ No tiene facturas para pagar.")
        return
    ...
    print("💰 Su saldo actual: $" + str(usuario["Saldo"]))
    
    # Verificar si tiene saldo suficiente
    if usuario["Saldo"] >= total:
        print("✅ Pago realizado con éxito!")
        usuario["Saldo"] = usuario["Saldo"] - total
        usuario["FacturasAdeudadas"] = []  # Borrar todas las facturas
        print("💰 Nuevo saldo: $" + str(usuario["Saldo"]))
    else:
        faltante = total - usuario["Saldo"]
        print("❌ Saldo insuficiente. Le faltan $" + str(faltante) + " para pagar todas las facturas.")
...
def sacarTurno():
    ...
    turno_random = random.choice(horarios)
    print("✅ Turno asignado para el día " + fecha + " a las " + turno_random + " hs.")
    print("📌 Por favor, llegue 10 minutos antes con su DNI.")
...
if usuarioActual is None:
    print("Lo siento, no podemos continuar. DNI no registrado.")
else:
